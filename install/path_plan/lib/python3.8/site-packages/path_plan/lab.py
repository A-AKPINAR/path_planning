...
'''
    ...
    def create_wall(self, x, y, orientation, ox=0, oy=0, size = 0.1):
        if orientation == "h":
            ori = 0
            # x_ = x+0.5 + ox
    ...
    def create_joint(self, x, y, ox=0, oy=0, size = 0.1):
        x = x*size+ox
        y = -(y*size+oy)
...
def main():
    ...
    usage="map_gen.py -s 10,10 -o 0,0 -l 1 -f maze -v\n"
    "       For negative origin use '=' sign\n"
    "       map_gen.py -s 5,5 -o=-5,-5 -l 2 -f maze -v")

    parser.add_argument("-s", "--size", metavar="",type=str , default="10,10", help="specifies size of maze as MxN. By default <15,15>") #changed to 10
