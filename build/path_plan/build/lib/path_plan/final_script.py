...
from .detect_robot import find_robot
from .robot_map import mapper
from .pathplanning import pathplan
from .motion_plan import motionplanner
...
from .features import Debugging
...
class maze_solver(Node):
    ...
    def __init__(self):
        ...
        # Visualizing what the robot sees 
        ...
        self.robot_localizer = find_robot()
        self.bot_mapper = mapper()
        self.bot_pathplanner = pathplan()
        self.bot_motionplanner = motionplanner()
    ...
    def process_data_bot(self, data):
      self.bot_view = self.bridge.imgmsg_to_cv2(data,'bgr8') 
    ...
    def overlay_live(self,frame_disp,overlay,overlay_map,overlay_path):
        ...
        crop_loc_col = self.robot_localizer.transform_arr[0]+self.bot_mapper.crp_amt

        crop_loc_row = self.robot_localizer.transform_arr[1]+self.bot_mapper.crp_amt

        new_cols = int(overlay_rot.shape[1]*robot_setup.debug_live_amount)
        new_path_cols = int(overlay_rot.shape[1]*robot_setup.debug_path_live_amount)
        new_map_cols = int(overlay_rot.shape[1]*robot_setup.debug_map_live_amount)


        frame_disp[crop_loc_row:crop_loc_row + overlay_rot.shape[0],crop_loc_col:crop_loc_col + new_cols] = overlay_rot[:,0:new_cols]
        
        if robot_setup.debug_map_live_amount>0:
            self.overlay_cropped(frame_disp,map_rot,crop_loc_row,crop_loc_col,new_map_cols)
        if robot_setup.debug_path_live_amount>0:
    ...
    def maze_solving(self):
        ...
        # Creating frame to display current robot state        
        frame_disp = self.sat_view.copy()
        
        # Localizing robot at each iteration        
        self.robot_localizer.localize_bot(self.sat_view, frame_disp)

        # Converting Image to Graph
        self.bot_mapper.graphify(self.robot_localizer.env_og)
        ...
        if robot_setup.debug and robot_setup.debug_pathplanning:
            print("\ Visited Nodes [Dijisktra V A-Star*] = [ {} V {} ]".format(self.bot_pathplanner.dijisktra.dijiktra_nodes_visited,self.bot_pathplanner.astar.astar_nodes_visited))


        #Reach the exit by navigating the path previously computed
        bot_loc = self.robot_localizer.loc_robot
        path = self.bot_pathplanner.path_to_goal
        self.bot_motionplanner.nav_path(bot_loc, path, self.vel_msg, self.velocity_publisher)

        # Displaying robot in env live
        img_shortest_path = self.bot_pathplanner.img_shortest_path
        self.bot_motionplanner.display_control_mechanism_in_action(bot_loc, path, img_shortest_path, self.robot_localizer, frame_disp)
        
        #  robot view --> on left to frame Display
        ...
        if robot_setup.debug_live:
            self.overlay_live(frame_disp,img_shortest_path,self.bot_mapper.robot_cnnct,self.bot_pathplanner.choosen_route)

        orig_col = 5 + int(bot_view.shape[1]/4)
        orig = (orig_col,bot_offset-5)
        cv2.putText(frame_disp, "Robot View", orig, cv2.FONT_HERSHEY_PLAIN, 2, (255,0,0),3)
        frame_disp = cv2.rectangle(frame_disp, (5,bot_offset), (bot_view.shape[1]+5,(bot_view.shape[0]+bot_offset)), (0,0,255),12)
        frame_disp[bot_offset:(bot_view.shape[0]+bot_offset),4:bot_view.shape[1]+4] = bot_view
 
        cv2.imshow("Gazebo Live", frame_disp)
        cv2.waitKey(1)
